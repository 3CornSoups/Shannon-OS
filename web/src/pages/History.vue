        ...
        <table class="data-table mobile-cards">
          ...
          <tbody>
            <tr v-for="record in filteredHistory" :key="record.id" class="table-row">
              <td class="table-cell" data-label="时间">{{ formatDate(record.created_at) }}</td>
              <td class="table-cell" data-label="服务器">{{ getServerName(record.host_id) }}</td>
              <td class="table-cell" data-label="模式">
                <span class="badge" :class="`badge-${record.mode}`">{{ record.mode.toUpperCase() }}</span>
              </td>
              <td class="table-cell text-truncate" data-label="用户请求" :title="record.user_prompt">{{ record.user_prompt }}</td>
              <td class="table-cell" data-label="状态">
                <span class="badge" :class="`badge-${record.status}`">{{ record.status }}</span>
              </td>
              <td class="table-cell" data-label="操作">
...
// 数据来自 historyStore（fetchActions 从 /api/history/actions/{host_id} 加载）
const filteredHistory = computed(() => historyStore.filteredHistory)

onMounted(async () => {
  await serverStore.fetchServers()
  servers.value = serverStore.servers
  // 加载当前服务器的操作记录；未选择服务器时展示空态
  if (serverStore.currentServer) {
    await historyStore.fetchActions(serverStore.currentServer.id)
  }
})
